...
Client side routing reroutes on the client side, so the page itself doesn't reload, only specific components. 
On client side routing there aren't very good ways to optimize for SEO, because most SEO tracking happens on the server side.
